...
class NexonServiceImpl(
        ...
        return try {
            ...
            val result = when {
                ...
                ocid.startsWith("Nexon API 서버 오류 발생") -> ResponseDto("Nexon API 서버 오류 발생")
                    ...
                    try {
                        val symbolResult = getSymbol(ocid).block() ?: "API 오류 발생"
    ...
    private fun getSymbol(ocid: String): Mono<String> {
    ...
    private fun formatSymbolInfo(symbols: List<SymbolDto>): String {
        // 심볼 이름과 축약어 매핑
